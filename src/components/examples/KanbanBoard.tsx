...
export function KanbanBoard() {
  ...
  const handleTaskDrop = (item: any, sourceColumnId: string | null, targetColumnId: string) => {
    ...
    const taskIndex = newColumns[sourceColumnIndex].tasks.findIndex(task => task.id === item.id);
  ...
  return (
    <DragDropProvider>
      <div className="flex flex-col h-full">
        <h2 className="text-2xl font-bold mb-6 text-gray-800">Kanban Board Example</h2>
        <p className="mb-4 text-gray-600">Drag and drop tasks between columns</p>
      ...
      <DragPreview
        render={item => (
          <div className="bg-white rounded-md shadow-lg p-3 border border-gray-300 w-64">
            ...
            <span className={`mt-1 inline-block text-xs px-2 py-1 rounded-full ${priorityColors[item.data.priority]}`}>
              {item.data.priority}
            </span>
          </div>
        )}
